...
*mechanochemistry is its primary subject* — any methodology (experimental,
computational/DFT, machine learning) qualifies, but neighbouring fields that
merely share the "mechano-" root or the word "grinding" (mechanobiology,
mechanotransduction, machining, engine tribology) are rejected.
...
# Minimum evidence that mechanochemistry is the primary subject: one
# unambiguous term ("mechanochem…", "ball mill…", "mechanoredox"...) suffices;
...
def classify(record: RawRecord) -> Classification:
    ...
    if prim_pts < _MIN_PRIMARY_POINTS:
        return Classification(False, 0, reason="no mechanochemistry evidence")
    ...
    # paper reads as "mechanochemistry topic + supporting details".
    ...
    if "General Mechanochemistry" in categories and len(categories) > 1:
        categories.remove("General Mechanochemistry")
