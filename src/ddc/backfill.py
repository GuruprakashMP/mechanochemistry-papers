"""Historical backfill — index the whole history of mechanochemistry.

Walks OpenAlex (which aggregates Crossref, PubMed, arXiv and more, with
abstracts for classification) using cursor pagination:

* **Topic sweep**: a set of broad mechanochemistry search queries, one year at
  a time, oldest publications to newest.
