...
primary mechanochemistry terms × support terms, with negative terms for the
neighbouring fields that share the "mechano-"/"grinding" vocabulary
(mechanobiology, mechanotransduction, machining, engine tribology) and a bonus
for chemistry/materials venues.
...
Score shape: `20 + 4·min(primary,10) + 3·min(support,12) + venue − penalties`,
...
* All links are **relative**, so the site works at any base path
  (e.g. `/repo/mechanochemistry_papers/`).

### Generated site lives at the project root
`index.html` etc. sit next to `src/` and `data/` so that copying the single
`mechanochemistry_papers/` folder into a Pages repository is the whole deployment. The
...
~23 broad-recall topic queries per year (precision comes from the classifier,
